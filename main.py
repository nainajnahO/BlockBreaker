...
def build_level():
    lst = []
    for y in range(0, window_y // 3, window_y // 20):
        for x in range(0, window_x - window_x // 10, window_x // 10):
            c = random.randint(50, 255)
            lst.append(draw_rect((c, c, c), x, y, window_x // 10, window_y // 20))
    return lst
...
game_over_txt = myfont.render('GAME OVER - r) restart', True, WHITE)
...
def main(platform_x_pos, ball_xpos, ball_ypos):
    ...
    while loop:
        ...
        if (platform_x_pos) <= ball_xpos <= (platform_x_pos + platform_w) and (
                platform_start_posy) <= ball_bot_side <= (platform_start_posy + platform_h):
            ball_ypos -= ball_speed
            GO_UP = True
        else:
            if GO_UP:
